...
BetaBirchFeature::BetaBirchFeature() : BetaTreeFeature() {}

bool BetaBirchFeature::place(Level* level, Random& random, const BlockPos& pos) {
...
bool BetaBirchFeature::canPlaceTree(Level* level, const BlockPos& pos, int height) {
...
void BetaBirchFeature::generateLeaves(Level* level, Random& random, const BlockPos& pos,
                                      int height) {
        ...
        for (int x = pos.x - radius; x <= pos.x + radius; ++x) {
            ...
            for (int z = pos.z - radius; z <= pos.z + radius; ++z) {
                ...
                if (std::abs(xOffset) != radius || std::abs(zOffset) != radius ||
                    ...
                    if (block == nullptr || block == Blocks::AIR || block == Blocks::LEAVES) {
                        level->setBlock(leafPos, Blocks::LEAVES->getBlockState(2), 2, false);
                    }
                }
            }
        }
...
void BetaBirchFeature::generateTrunk(Level* level, const BlockPos& pos, int height) {
    for (int y = 0; y < height; ++y) {
        BlockPos trunkPos(pos.x, pos.y + y, pos.z);
        const Block* block = level->getBlockState(trunkPos)->getBlock();
        if (block == nullptr || block == Blocks::AIR || block == Blocks::LEAVES) {
            level->setBlock(trunkPos, Blocks::LOG->getBlockState(2), 2, false);
        }
    }
}
